...
const CVPreview = ({ user, aiData }) => {
  ...
  // --- SUMMARY (ÇOKLU VERSİYON DESTEĞİ) ---

  const baseSummary =
    aiData?.summary || user.summary || user.aboutMe || "";

  const summaries =
    aiData?.summaries && aiData.summaries.length > 0
      ? aiData.summaries
      : baseSummary
      ? [baseSummary]
      : [];
  ...
  const userData = {
    ...
    summary: currentSummary, // 🔥 Artık seçili summary buradan geliyor
  ...
  <section className="cv-section">
    {/* EXPERIENCE ile aynı h2 yapısı */}
    <h2 className="section-title">
      SUMMARY {aiData ? "(AI Optimized)" : ""}
    </h2>

    {/* Buton alt satırda, sağa hizalı */}
    {summaries.length > 1 && (
      <div className="summary-button-wrapper pdf-exclude">
        <button
          type="button"
